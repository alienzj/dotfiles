...
in {
  ...
  modules.services.waybar.settings = {
    main = {
      ...
      modules-right = [
        ...
        "cpu"
        (let
          hw = config.modules.profiles.hardware;
        in
  ...
  modules.desktop.hyprland.mako.settings = {
    ...
    height = 300;
    icon-path = let
      iconDir = "/etc/profiles/per-user/${config.user.name}/share/icons";
    in
      concatStringsSep ":" [
  ...
  modules.desktop.hyprland.hyprlock.settings = let
    monitor = (findFirst (x: x.primary) {} config.modules.desktop.hyprland.monitors).output or "";
  in {
    backgrounds = [
      {
